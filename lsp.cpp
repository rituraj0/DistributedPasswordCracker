...
//			check the connection ID of the received message, if it is zero this is new client
      		if(receivedMessage.connid == 0 || (receivedMessage.connid !=0 && receivedMessage.seqnum ==0))// this is a new connection request or a repeated
//                                                        connection request for some previous client because of the keep alive protocol

      		{
//      			check if this client is already connected, this might happen because
//                    the client might send more then one connection request because of the 					keep alive
...
//				printf("The first message recceived with a sequence number=%d\n", receivedMessage.seqnum);
	...
	//				create a an Ack for this received data message and enqueue it directly in the outbox queue to be sent soon by the sending thread
			...
			else if (receivedMessage.seqnum == main_Server.server_connection_map.at(receivedMessage.connid).last_data_received_seqnum +1) //set the message type
                                 //to 2, and enqueue it in the inbox queue to be read by the server
			{
